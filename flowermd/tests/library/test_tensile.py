...
class TestTensileSimulation(BaseTest):
    ...
    def test_log_data(self):
        ...
        assert len(tensile_sim._stress_logs[0][1:]) == 100
    ...
    def test_compile_log_data(self):
        pps = PPS(lengths=6, num_mols=32)
        system = Lattice(
            molecules=[pps],
            x=1.2,
        ...
        assert np.allclose(tensile_sim.strain, 0.05, 1e-4)
        strain, stress_avg, stress_std = (
            tensile_sim.compile_stress_strain_data()
        )
        assert len(strain) == 10
        assert strain.shape == stress_avg.shape == stress_std.shape
