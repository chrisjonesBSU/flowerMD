...
class TestTensileSimulation(BaseTest):
    ...
    def test_log_data(self):
        ...
        assert len(tensile_sim._strain_logs[0][1:]) == 10
        tensile_sim.run_tensile(
            strain=0.05, kT=2.0, n_steps=5e2, period=10, tau_kt=0.001
        )
        assert len(tensile_sim._strain_logs) == 2
        assert len(tensile_sim._stress_logs) == 2
        assert len(tensile_sim._strain_logs[2][1:]) == 5
